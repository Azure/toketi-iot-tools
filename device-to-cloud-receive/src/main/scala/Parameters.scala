...
case class Parameters(
    ...
    hubNamespace: String = Configuration.hubNamespace,
    hubName: String = Configuration.hubName,
  ...
  def build: OptionParser[Parameters] = {
    new scopt.OptionParser[Parameters]("d2c-send") {
      ...
      opt[String]('n', "namespace").optional.valueName("<ns>").action((x, c) => c.copy(hubNamespace = x)).text("Azure IoT Hub namespace")
      ...
      opt[String]('p', "policy").optional.valueName("<name>").action((x, c) => c.copy(accessPolicy = x)).text("Azure IoT Hub shared access policy")
      opt[String]('k', "key").optional.valueName("<key>").action((x, c) => c.copy(hubConsumerGroup = x)).text("Azure IoT Hub shared access key")
      ...
      note("IOTHUB_NAMESPACE, IOTHUB_NAME, IOTHUB_ACCESS_POLICY and IOTHUB_ACCESS_KEY")
    }
  }
}
